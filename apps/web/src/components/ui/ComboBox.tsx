        ...
        >
          <div className={cn('flex flex-col', maxHeight)}>
            ...
            {showSelectAll && (onSelectAll || onClearAll) && (
              <ComboBoxActions onSelectAll={onSelectAll} onClearAll={onClearAll} />
            )}
...
function ComboBoxList<T>({
  ...
  return (
    ...
    >
      {options.length > 0
        ? options.map(option => (
            <ComboBoxItem key={String(option.value)} option={option} renderItem={renderItem} />
          ))
        : showEmptyMessage && (
    ...
    >
      {OptionIcon ? (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-dark-700 dark:text-dark-200">{option.label}</span>
        </div>
      ) : (
        <span className="text-sm text-dark-700 dark:text-dark-200">{option.label}</span>
      )}
