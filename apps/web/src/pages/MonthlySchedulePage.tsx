...
export default function MonthlySchedulePage() {
  ...
  const [selectedServiceForDraw, setSelectedServiceForDraw] = useState<string>('')
  ...
  function handleDrawMonthly(serviceId: string) {
    ...
    if (activeTeams.length === 0) {
      setAlertConfig({
        isOpen: true,
        type: 'warning',
        title: 'Sem Equipes Ativas',
        message:
          'Não há equipes ativas disponíveis para este culto. Por favor, ative pelo menos uma equipe antes de sortear.',
      })
      return
    }
    ...
    const updatedSchedules = schedules.filter(s => !existingScheduleIds.has(s.id))
          ...
          <div className="space-y-4">
            <p className="text-dark-300">
              ...
              <span className="font-medium">
                {selectedServiceForDraw ? getServiceName(selectedServiceForDraw) : ''}
              </span>{' '}
              no mês de <span className="font-medium">{monthName}</span>.
            </p>
            <p className="text-sm text-dark-400">
              As escalas existentes para este mês serão substituídas pelas novas escalas sorteadas.
            </p>
